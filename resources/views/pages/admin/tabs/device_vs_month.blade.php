<form action="{{ route('admin.heatmap.device-vs-month') }}">
    <div class="row">
                ...
                <div class="card-body overflow-y-auto" style="max-height: 20rem;">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="devices[]"
                            value="{{ $device->device_name }}" id="{{ $device->device_name }}" checked>
                        <label class="form-check-label" for="{{ $device->device_name }}">
                            {{ strtoupper($device->device_name) }}
                        </label>
                    </div>
                    @endforeach
                </div>
