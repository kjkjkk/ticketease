            ...
            <div class="row p-3 d-flex align-items-center rounded">
                <div class="col-sm-12 col-md-6 col-xl-3 mb-3">
                    ...
                    <form action="{{ route('admin.history') }}" method="GET" class="d-flex">
                        ...
                        <button type="submit" class="btn btn-logo ms-1"><i class="fi fi-ss-member-search"></i></button>
                    </form>
                </div>
                <div class="col-sm-12 col-md-6 col-xl-3 mb-3">
                ...
                <div class="col-sm-12 col-md-4 col-xl-3 mb-3">
                    ...
                    <x-dropdown title="{{ request('ticket_nature_name') ? request('ticket_nature_name') : 'All' }}"
                        route="{{ route('admin.history') }}">
                        @foreach ($ticketNatures as $ticket_nature)
                        <a class="dropdown-item text-truncate"
                            href="{{ route('admin.history', array_merge(request()->query(), ['ticket_nature_name' => $ticket_nature->ticket_nature_name])) }}">
                ...
                <div class="col-sm-12 col-md-4 col-xl-3 mb-3">
                    <small class="fw-semibold">Filter by technician</small>
                    @php
                    $selectedTechnician = request('technician_id') ? $technicians->firstWhere('id',
                    request('technician_id')) : null;
                    @endphp
                    <x-dropdown
                        title="{{ $selectedTechnician ? $selectedTechnician->firstname . ' ' . $selectedTechnician->lastname : 'All' }}"
                        route="{{ route('admin.history') }}">
                        @foreach ($technicians as $technician)
                        <a class="dropdown-item text-truncate"
                            href="{{ route('admin.history', array_merge(request()->query(), ['technician_id' => $technician->id])) }}">
    ...
    <div class="table-responsive mt-3 rounded bg-white border p-3">
        <table class="table table-bordered user-table">
            <thead class="table-logo-dark">
                <tr>
                    @foreach (['#', 'Requestor', 'Ticket Nature', 'District', 'Technician', 'Status'] as $header)
                    <th>{{ $header }}</th>
                    @endforeach
